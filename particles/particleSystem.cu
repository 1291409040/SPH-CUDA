                     ...
                     uint numParticles)
{
	...
	thrust::device_ptr<float4> d_posTmp4((float4 *)posTmp);
    thrust::device_ptr<float4> d_vel4((float4 *)vel);

    thrust::for_each(
        thrust::make_zip_iterator(thrust::make_tuple(d_pos4, d_posTmp4, d_vel4)),
        thrust::make_zip_iterator(thrust::make_tuple(d_pos4+numParticles, d_posTmp4+numParticles, d_vel4+numParticles)),
               ...
               uint   numCells)
{
    ...
    // execute the kernel
    calLambdaD<<< numBlocks, numThreads >>>((float4*)sortedPosTmp,
                                          (float*)lambda,
                                          gridParticleIndex,
                                          cellStart,
                                          cellEnd,
               ...
               uint   numCells)
{
    ...
    // execute the kernel
    calDeltaP_Collision_UpdatePosD<<< numBlocks, numThreads >>>((float4*)sortedPosTmp,
										  (float4*)sortedPosTmp2,
                                          (float*)lambda,
                                          gridParticleIndex,
                                          cellStart,
                                          cellEnd,
                                          numParticles);
	cutilSafeCall(cudaMemcpy(sortedPosTmp, sortedPosTmp2, sizeof(float4) * numParticles, cudaMemcpyDeviceToDevice));
				  ...
				  uint   numCells)
{
    ...
    // execute the kernel
    updateVelPosD<<< numBlocks, numThreads >>>(deltaTime,
										  (float4*)newPos,
                                          (float4*)newVel,
                                          (float4*)sortedPosTmp,
                                          gridParticleIndex,
